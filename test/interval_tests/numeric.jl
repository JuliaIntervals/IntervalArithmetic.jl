...
@testset "Power tests" begin
    ...
    @test isweakinterior(interval(1., 3.), interval(1,27)^(1//3))
...
@testset "Exp and log tests" begin
    @test isweakinterior(exp(interval(BigFloat, 1//2)), exp(interval(1//2)))
    @test ismember(exp(big(1//2)), exp(interval(1//2)))
    @test isweakinterior(exp(interval(BigFloat, 0.1)), exp(interval(0.1)))
    ...
    @test isweakinterior(log(interval(BigFloat, 1//2)), log(interval(1//2)))
    @test ismember(log(big(1//2)), log(interval(1//2)))
    @test isweakinterior(log(interval(BigFloat, 0.1)), log(interval(0.1)))
    ...
    @test isweakinterior(exp2(interval(BigFloat, 1//2)), exp2(interval(1//2)))
    @test isequalinterval(exp2(interval(1024.0)), interval(1.7976931348623157e308, Inf))
    @test isweakinterior(exp10(interval(BigFloat, 1//2)), exp10(interval(1//2)))
    @test isequalinterval(exp10(interval(308.5)), interval(1.7976931348623157e308, Inf))

    @test isweakinterior(log2(interval(BigFloat, 1//2)), log2(interval(1//2)))
...
@testset "Rational tests" begin
    ...
    @test isweakinterior(interval(f*g), interval(1)/9)
end
...
@testset "Fast power" begin
    ...
    @testset "Fast integer powers" begin
        ...
        x = interval(pi)
        @test isweakinterior(x^100, pow(x, 100))
        @test isweakinterior(x^50, pow(x, 50))
        @test isstrictinterior(x^50, pow(x, 50))
    ...
    @testset "Fast real powers" begin
        ...
        @test isweakinterior(y^2.1, pow(y, 2.1))
    end
    ...
    @testset "cbrt" begin
        ...
        @test isweakinterior(cbrt(big(interval(2, 3))), cbrt(interval(2, 3)))
        ...
        @test isweakinterior(cbrt(interval(2, 3)), cbrt(interval(2f0, 3f0)))
    end
