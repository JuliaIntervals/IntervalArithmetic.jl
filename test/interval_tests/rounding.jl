...
@testset "IntervalRounding{:slow}" begin
    ...
    @test isequalinterval(sin(x), interval(0.47942553860420295, 0.479425538604203))
end

@testset "IntervalRounding{:accurate}" begin
    IntervalArithmetic.interval_rounding() = IntervalRounding{:accurate}()
    @test isequalinterval(sin(x), interval(0.47942553860420295, 0.47942553860420306))
end

@testset "No rounding" begin
    IntervalArithmetic.interval_rounding() = IntervalRounding{:none}()
    @test isequalinterval(sin(x), interval(0.479425538604203, 0.479425538604203))
end
...
@testset "IntervalRounding{:tight}" begin
    ...
    @test isequalinterval(tiny * tiny, interval(0, nextfloat(0.0)))
    @test isequalinterval(huge * huge, interval(floatmax(), Inf))
    @test isequalinterval(huge / tiny, interval(floatmax(), Inf))
    @test isequalinterval(tiny / huge, interval(0, nextfloat(0.0)))
end
