...
@testset "minimal_nums_to_decorated_interval_test" begin
    ...
    @test_logs (:warn,) @test isnai(interval(NaN,NaN))

    @test_logs (:warn,) @test isnai(interval(1.0,-1.0))

    @test_logs (:warn,) @test isnai(interval(-Inf,-Inf))
...
@testset "minimal_text_to_decorated_interval_test" begin
    ...
    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[ Nai  ]_ill"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[ Nai  ]_trv"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[ Empty  ]_ill"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[  ]_com"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[,]_com"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[   Entire ]_com"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[ -inf ,  INF ]_com"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[  -1.0  , 1.0]_ill"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[  -1.0  , 1.0]_fooo"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[  -1.0  , 1.0]_da"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[-1.0,]_com"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[-Inf, 1.000 ]_ill"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[-I  nf, 1.000 ]"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[-Inf, 1.0  00 ]"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[-Inf ]"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[Inf , INF]"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[ foo ]"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "0.0??_com"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "0.0??u_ill"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "0.0??d_com"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "0.0??_com"))

    @test_logs (:warn,) @test isnai(parse(Interval{Float64}, "[1.0,2.0"))
...
@testset "minimal_interval_part_test" begin
    ...
    @test_logs (:warn,) @test bareinterval(nai()) === emptyinterval(BareInterval{Float64})
...
@testset "minimal_set_dec_test" begin
    ...
    @test_logs (:warn,) @test isnai(interval(emptyinterval(BareInterval{Float64}), ill))

    @test_logs (:warn,) @test isnai(interval(bareinterval(-Inf,3.0), ill))

    @test_logs (:warn,) @test isnai(interval(bareinterval(-1.0,3.0), ill))
