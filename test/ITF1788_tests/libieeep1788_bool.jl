...
@testset "minimal_equal_dec_test" begin
    ...
    @test isequal_interval(interval(emptyinterval(BareInterval{Float64}), trv), nai()) === false

    @test isequal_interval(nai(), interval(emptyinterval(BareInterval{Float64}), trv)) === false
    ...
    @test isequal_interval(nai(), interval(bareinterval(1.0,2.0), trv)) === false
...
@testset "minimal_subset_dec_test" begin
    ...
    @test issubset_interval(nai(), interval(emptyinterval(BareInterval{Float64}), trv)) === false

    @test issubset_interval(nai(), interval(bareinterval(0.0,4.0), trv)) === false
...
@testset "minimal_less_dec_test" begin
    ...
    @test isweakless(interval(emptyinterval(BareInterval{Float64}), trv), nai()) === false

    @test isweakless(interval(bareinterval(1.0,2.0), trv), nai()) === false

    @test isweakless(nai(), interval(bareinterval(1.0,2.0), def)) === false
...
@testset "minimal_precedes_dec_test" begin
    ...
    @test precedes(nai(), interval(bareinterval(3.0,4.0), def)) === false

    @test precedes(interval(bareinterval(3.0,4.0), trv), nai()) === false

    @test precedes(nai(), interval(emptyinterval(BareInterval{Float64}), trv)) === false
...
@testset "minimal_interior_dec_test" begin
    ...
    @test isinterior(nai(), interval(emptyinterval(BareInterval{Float64}), trv)) === false

    @test isinterior(nai(), interval(bareinterval(0.0,4.0), trv)) === false

    @test isinterior(interval(bareinterval(0.0,4.0), def), nai()) === false
...
@testset "minimal_strictly_less_dec_test" begin
    ...
    @test isstrictless(interval(emptyinterval(BareInterval{Float64}), trv), nai()) === false

    @test isstrictless(interval(bareinterval(1.0,2.0), trv), nai()) === false

    @test isstrictless(nai(), interval(bareinterval(1.0,2.0), def)) === false
...
@testset "minimal_strictly_precedes_dec_test" begin
    ...
    @test strictprecedes(nai(), interval(bareinterval(3.0,4.0), trv)) === false

    @test strictprecedes(interval(bareinterval(3.0,4.0), def), nai()) === false

    @test strictprecedes(nai(), interval(emptyinterval(BareInterval{Float64}), trv)) === false
...
@testset "minimal_disjoint_dec_test" begin
    ...
    @test isdisjoint_interval(nai(), interval(bareinterval(3.0,4.0), def)) === false

    @test isdisjoint_interval(interval(bareinterval(3.0,4.0), trv), nai()) === false

    @test isdisjoint_interval(interval(emptyinterval(BareInterval{Float64}), trv), nai()) === false
