...
@testset "Mince and hull" begin
    ...
    @test mapreduce((x, y) -> all(x .≛ y), &, vb2, vv)
    @test all(hull.(vb2...) .≛ ib2)
    @test mapreduce((x, y) -> all(x .≛ y), &, mince(ib2, (4, 4)), vb2)
    @test mapreduce((x, y) -> all(x .≛ y), &, mince(ib2, (1,4)),
    ...
    vb3 = mince(ib3, 4)
    @test length(vb3) == 4^3
    @test all(hull.(vb3...) .≛ ib3)
    @test mapreduce((x, y) -> all(x .≛ y), &, mince(ib3, (4,4,4)), vb3)
    @test mapreduce((x, y) -> all(x .≛ y), &, mince(ib3, (2,1,1)),
        [[(-1 .. 0), (-1 .. 1), (-1 .. 1)], [(0 .. 1), (-1 .. 1), (-1 .. 1)]])
    @test all(hull.(mince(ib3, (2,1,1))...) .≛ ib3)

    ib4 = fill(-1..1, 4)
    vb4 = mince(ib4, 4)
    @test length(vb4) == 4^4
    @test all(hull.(vb4...) .≛ ib4)
    @test mapreduce((x, y) -> all(x .≛ y), &, mince(ib4, (4,4,4,4)), vb4)
    @test mapreduce((x, y) -> all(x .≛ y), &, mince(ib4, (1,1,1,1)), (ib4,))
end
