...
@testset "minimal_text_to_interval_test" begin
    ...
    @test parse(Interval{Float64}, "[ Empty  ]_trv") === emptyinterval()

    @test parse(Interval{Float64}, "[  ]") === emptyinterval()

    @test parse(Interval{Float64}, "[  ]_trv") === emptyinterval()

    @test parse(Interval{Float64}, "[,]") === interval(-Inf,Inf)

    @test parse(Interval{Float64}, "[,]_trv") === emptyinterval()

    @test parse(Interval{Float64}, "[ entire  ]") === interval(-Inf,Inf)

    @test parse(Interval{Float64}, "[ ENTIRE ]_dac") === emptyinterval()

    @test parse(Interval{Float64}, "[ ENTIRE ]") === interval(-Inf,Inf)

    @test parse(Interval{Float64}, "[ -inf , INF  ]") === interval(-Inf,Inf)

    @test parse(Interval{Float64}, "[ -inf, INF ]_def") === emptyinterval()

    @test parse(Interval{Float64}, "[-1.0,1.0]") === interval(-1.0,1.0)

    @test parse(Interval{Float64}, "[  -1.0  ,  1.0  ]") === interval(-1.0,1.0)

    @test parse(Interval{Float64}, "[  -1.0  , 1.0]") === interval(-1.0,1.0)

    @test parse(Interval{Float64}, "[-1,]") === interval(-1.0,Inf)

    @test parse(Interval{Float64}, "[-1.0, +inf]") === interval(-1.0,Inf)

    @test parse(Interval{Float64}, "[-1.0, +infinity]") === interval(-1.0,Inf)

    @test parse(Interval{Float64}, "[-Inf, 1.000 ]") === interval(-Inf,1.0)

    @test parse(Interval{Float64}, "[-Infinity, 1.000 ]") === interval(-Inf,1.0)

    @test parse(Interval{Float64}, "[1.0E+400 ]") === interval(0x1.fffffffffffffp+1023,Inf)

    @test parse(Interval{Float64}, "[ -4/2, 10/5 ]") === interval(-2.0,2.0)

    @test parse(Interval{Float64}, "[ -1/10, 1/10 ]") === interval(-0.1,0.1)

    @test parse(Interval{Float64}, "0.0?") === interval(-0.05,0.05)

    @test parse(Interval{Float64}, "0.0?u") === interval(0.0,0.05)

    @test parse(Interval{Float64}, "0.0?d") === interval(-0.05,0.0)

    @test parse(Interval{Float64}, "2.5?") === interval(0x1.3999999999999p+1,0x1.4666666666667p+1)

    @test parse(Interval{Float64}, "2.5?u") === interval(2.5,0x1.4666666666667p+1)

    @test parse(Interval{Float64}, "2.5?d") === interval(0x1.3999999999999p+1,2.5)

    @test parse(Interval{Float64}, "0.000?5") === interval(-0.005,0.005)

    @test parse(Interval{Float64}, "0.000?5u") === interval(0.0,0.005)

    @test parse(Interval{Float64}, "0.000?5d") === interval(-0.005,0.0)

    @test parse(Interval{Float64}, "2.500?5") === interval(0x1.3f5c28f5c28f5p+1,0x1.40a3d70a3d70bp+1)

    @test parse(Interval{Float64}, "2.500?5u") === interval(2.5,0x1.40a3d70a3d70bp+1)

    @test parse(Interval{Float64}, "2.500?5d") === interval(0x1.3f5c28f5c28f5p+1,2.5)

    @test parse(Interval{Float64}, "0.0??") === interval(-Inf,Inf)

    @test parse(Interval{Float64}, "0.0??u") === interval(0.0,Inf)

    @test parse(Interval{Float64}, "0.0??d") === interval(-Inf,0.0)

    @test parse(Interval{Float64}, "2.5??") === interval(-Inf,Inf)

    @test parse(Interval{Float64}, "2.5??u") === interval(2.5,Inf)

    @test parse(Interval{Float64}, "2.5??d") === interval(-Inf,2.5)

    @test parse(Interval{Float64}, "2.500?5e+27") === interval(0x1.01fa19a08fe7fp+91,0x1.0302cc4352683p+91)
    ...
    @test parse(Interval{Float64}, "10?3") === interval(7.0,13.0)

    @test parse(Interval{Float64}, "10?3e380") === interval(0x1.fffffffffffffp+1023,Inf)

    @test parse(Interval{Float64}, "1.0000000000000001?1") === interval(1.0,0x1.0000000000001p+0)
    ...
    @test parse(Interval{Float64}, "[ Nai  ]") === emptyinterval()

    @test parse(Interval{Float64}, "[ Nai  ]_ill") === emptyinterval()

    @test parse(Interval{Float64}, "[ Nai  ]_trv") === emptyinterval()

    @test parse(Interval{Float64}, "[ Empty  ]_ill") === emptyinterval()

    @test parse(Interval{Float64}, "[  ]_com") === emptyinterval()

    @test parse(Interval{Float64}, "[,]_com") === emptyinterval()

    @test parse(Interval{Float64}, "[   Entire ]_com") === emptyinterval()

    @test parse(Interval{Float64}, "[ -inf ,  INF ]_com") === emptyinterval()

    @test parse(Interval{Float64}, "[  -1.0  , 1.0]_ill") === emptyinterval()

    @test parse(Interval{Float64}, "[  -1.0  , 1.0]_fooo") === emptyinterval()

    @test parse(Interval{Float64}, "[  -1.0  , 1.0]_da") === emptyinterval()

    @test parse(Interval{Float64}, "[-1.0,]_com") === emptyinterval()

    @test parse(Interval{Float64}, "[-Inf, 1.000 ]_ill") === emptyinterval()

    @test parse(Interval{Float64}, "[-I  nf, 1.000 ]") === emptyinterval()

    @test parse(Interval{Float64}, "[-Inf, 1.0  00 ]") === emptyinterval()

    @test parse(Interval{Float64}, "[-Inf ]") === emptyinterval()

    @test parse(Interval{Float64}, "[Inf , INF]") === emptyinterval()

    @test parse(Interval{Float64}, "[ foo ]") === emptyinterval()

    @test parse(Interval{Float64}, "[1.0000000000000002,1.0000000000000001]") === interval(1.0,0x1.0000000000001p+0)

    @test parse(Interval{Float64}, "[10000000000000001/10000000000000000,10000000000000002/10000000000000001]") === interval(1.0,0x1.0000000000001p+0)

    @test parse(Interval{Float64}, "[0x1.00000000000002p0,0x1.00000000000001p0]") === interval(1.0,0x1.0000000000001p+0)

end

@testset "minimal_text_to_decorated_interval_test" begin

    @test parse(DecoratedInterval{Float64}, "[ Empty  ]") === DecoratedInterval(emptyinterval(), trv)

    @test parse(DecoratedInterval{Float64}, "[ Empty  ]_trv") === DecoratedInterval(emptyinterval(), trv)

    @test parse(DecoratedInterval{Float64}, "[  ]") === DecoratedInterval(emptyinterval(), trv)

    @test parse(DecoratedInterval{Float64}, "[  ]_trv") === DecoratedInterval(emptyinterval(), trv)

    @test parse(DecoratedInterval{Float64}, "[,]") === DecoratedInterval(entireinterval(), dac)

    @test parse(DecoratedInterval{Float64}, "[,]_trv") === DecoratedInterval(entireinterval(), trv)

    @test parse(DecoratedInterval{Float64}, "[ entire  ]") === DecoratedInterval(entireinterval(), dac)

    @test parse(DecoratedInterval{Float64}, "[ ENTIRE ]_dac") === DecoratedInterval(entireinterval(), dac)

    @test parse(DecoratedInterval{Float64}, "[ -inf , INF  ]") === DecoratedInterval(entireinterval(), dac)

    @test parse(DecoratedInterval{Float64}, "[ -inf, INF ]_def") === DecoratedInterval(entireinterval(), def)

    @test parse(DecoratedInterval{Float64}, "[-1.0,1.0]") === DecoratedInterval(interval(-1.0,1.0), com)

    @test parse(DecoratedInterval{Float64}, "[  -1.0  ,  1.0  ]_com") === DecoratedInterval(interval(-1.0,1.0), com)

    @test parse(DecoratedInterval{Float64}, "[  -1.0  , 1.0]_trv") === DecoratedInterval(interval(-1.0,1.0), trv)

    @test parse(DecoratedInterval{Float64}, "[-1,]") === DecoratedInterval(interval(-1.0,Inf), dac)

    @test parse(DecoratedInterval{Float64}, "[-1.0, +inf]_def") === DecoratedInterval(interval(-1.0,Inf), def)

    @test parse(DecoratedInterval{Float64}, "[-1.0, +infinity]_def") === DecoratedInterval(interval(-1.0,Inf), def)

    @test parse(DecoratedInterval{Float64}, "[-Inf, 1.000 ]") === DecoratedInterval(interval(-Inf,1.0), dac)

    @test parse(DecoratedInterval{Float64}, "[-Infinity, 1.000 ]_trv") === DecoratedInterval(interval(-Inf,1.0), trv)

    @test parse(DecoratedInterval{Float64}, "[1.0E+400 ]_com") === DecoratedInterval(interval(0x1.fffffffffffffp+1023,Inf), dac)

    @test parse(DecoratedInterval{Float64}, "[ -4/2, 10/5 ]_com") === DecoratedInterval(interval(-2.0,2.0), com)

    @test parse(DecoratedInterval{Float64}, "[ -1/10, 1/10 ]_com") === DecoratedInterval(interval(-0.1,0.1), com)

    @test parse(DecoratedInterval{Float64}, "0.0?") === DecoratedInterval(interval(-0.05,0.05), com)

    @test parse(DecoratedInterval{Float64}, "0.0?u_trv") === DecoratedInterval(interval(0.0,0.05), trv)

    @test parse(DecoratedInterval{Float64}, "0.0?d_dac") === DecoratedInterval(interval(-0.05,0.0), dac)

    @test parse(DecoratedInterval{Float64}, "2.5?") === DecoratedInterval(interval(0x1.3999999999999p+1,0x1.4666666666667p+1), com)

    @test parse(DecoratedInterval{Float64}, "2.5?u") === DecoratedInterval(interval(2.5,0x1.4666666666667p+1), com)

    @test parse(DecoratedInterval{Float64}, "2.5?d_trv") === DecoratedInterval(interval(0x1.3999999999999p+1,2.5), trv)

    @test parse(DecoratedInterval{Float64}, "0.000?5") === DecoratedInterval(interval(-0.005,0.005), com)

    @test parse(DecoratedInterval{Float64}, "0.000?5u_def") === DecoratedInterval(interval(0.0,0.005), def)

    @test parse(DecoratedInterval{Float64}, "0.000?5d") === DecoratedInterval(interval(-0.005,0.0), com)

    @test parse(DecoratedInterval{Float64}, "2.500?5") === DecoratedInterval(interval(0x1.3f5c28f5c28f5p+1,0x1.40a3d70a3d70bp+1), com)

    @test parse(DecoratedInterval{Float64}, "2.500?5u") === DecoratedInterval(interval(2.5,0x1.40a3d70a3d70bp+1), com)

    @test parse(DecoratedInterval{Float64}, "2.500?5d") === DecoratedInterval(interval(0x1.3f5c28f5c28f5p+1,2.5), com)

    @test parse(DecoratedInterval{Float64}, "0.0??_dac") === DecoratedInterval(interval(-Inf,Inf), dac)

    @test parse(DecoratedInterval{Float64}, "0.0??u_trv") === DecoratedInterval(interval(0.0,Inf), trv)

    @test parse(DecoratedInterval{Float64}, "0.0??d") === DecoratedInterval(interval(-Inf,0.0), dac)

    @test parse(DecoratedInterval{Float64}, "2.5??") === DecoratedInterval(interval(-Inf,Inf), dac)

    @test parse(DecoratedInterval{Float64}, "2.5??u_def") === DecoratedInterval(interval(2.5,Inf), def)

    @test parse(DecoratedInterval{Float64}, "2.5??d_dac") === DecoratedInterval(interval(-Inf,2.5), dac)

    @test parse(DecoratedInterval{Float64}, "2.500?5e+27") === DecoratedInterval(interval(0x1.01fa19a08fe7fp+91,0x1.0302cc4352683p+91), com)

    @test parse(DecoratedInterval{Float64}, "2.500?5ue4_def") === DecoratedInterval(interval(0x1.86ap+14,0x1.8768p+14), def)

    @test parse(DecoratedInterval{Float64}, "2.500?5de-5") === DecoratedInterval(interval(0x1.a2976f1cee4d5p-16,0x1.a36e2eb1c432dp-16), com)
    ...
    @test parse(DecoratedInterval{Float64}, "10?3_com") === DecoratedInterval(interval(7.0,13.0), com)

    @test parse(DecoratedInterval{Float64}, "10?3e380_com") === DecoratedInterval(interval(0x1.fffffffffffffp+1023,Inf), dac)
    ...
    @test parse(DecoratedInterval{Float64}, "[1.0000000000000002,1.0000000000000001]") === DecoratedInterval(interval(1.0,0x1.0000000000001p+0), com)

    @test parse(DecoratedInterval{Float64}, "[10000000000000001/10000000000000000,10000000000000002/10000000000000001]") === DecoratedInterval(interval(1.0,0x1.0000000000001p+0), com)

    @test parse(DecoratedInterval{Float64}, "[0x1.00000000000002p0,0x1.00000000000001p0]") === DecoratedInterval(interval(1.0,0x1.0000000000001p+0), com)
