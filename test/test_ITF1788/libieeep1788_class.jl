...
@testset "minimal_text_to_interval_test" begin
    ...
    @test_broken @interval("[,]_trv") === emptyinterval()

    @test @interval("[ entire  ]") === Interval(-Inf,Inf)

    @test_broken @interval("[ ENTIRE ]_dac") === emptyinterval()

    @test_broken @interval("[ ENTIRE ]") === Interval(-Inf,Inf)

    @test_broken @interval("[ -inf , INF  ]") === Interval(-Inf,Inf)

    @test_broken @interval("[ -inf, INF ]_def") === emptyinterval()
    ...
    @test_broken @interval("[-1.0, +inf]") === Interval(-1.0,Inf)

    @test @interval("[-1.0, +infinity]") === Interval(-1.0,Inf)

    @test @interval("[-Inf, 1.000 ]") === Interval(-Inf,1.0)

    @test_broken @interval("[-Infinity, 1.000 ]") === Interval(-Inf,1.0)

    @test_broken @interval("[1.0E+400 ]") === Interval(0x1.fffffffffffffp+1023,Inf)
    ...
    @test_broken @interval("2.500?5e+27") === Interval(0x1.01fa19a08fe7fp+91,0x1.0302cc4352683p+91)
    ...
    @test_broken @interval("10?3e380") === Interval(0x1.fffffffffffffp+1023,Inf)

    @test_broken @interval("1.0000000000000001?1") === Interval(1.0,0x1.0000000000001p+0)

    @test_broken @interval("10?") === Interval(-Inf,Inf)

    @test_broken @interval("[ Nai  ]") === emptyinterval()

    @test_broken @interval("[ Nai  ]_ill") === emptyinterval()

    @test_broken @interval("[ Nai  ]_trv") === emptyinterval()
    ...
    @test_broken @interval("[  -1.0  , 1.0]_ill") === emptyinterval()

    @test_broken @interval("[  -1.0  , 1.0]_fooo") === emptyinterval()

    @test_broken @interval("[  -1.0  , 1.0]_da") === emptyinterval()

    @test_broken @interval("[-1.0,]_com") === emptyinterval()

    @test_broken @interval("[-Inf, 1.000 ]_ill") === emptyinterval()

    @test_broken @interval("[-I  nf, 1.000 ]") === emptyinterval()

    @test_broken @interval("[-Inf, 1.0  00 ]") === emptyinterval()

    @test_broken @interval("[-Inf ]") === emptyinterval()

    @test_broken @interval("[Inf , INF]") === emptyinterval()

    @test_broken @interval("[ foo ]") === emptyinterval()

    @test_broken @interval("[1.0000000000000002,1.0000000000000001]") === Interval(1.0,0x1.0000000000001p+0)

    @test @interval("[10000000000000001/10000000000000000,10000000000000002/10000000000000001]") === Interval(1.0,0x1.0000000000001p+0)

    @test_broken @interval("[0x1.00000000000002p0,0x1.00000000000001p0]") === Interval(1.0,0x1.0000000000001p+0)
...
@testset "minimal_text_to_decorated_interval_test" begin
    ...
    @test_skip @decorated("[ -inf , INF  ]") === DecoratedInterval(entireinterval(), dac)

    @test_skip @decorated("[ -inf, INF ]_def") === DecoratedInterval(entireinterval(), def)
    ...
    @test_skip @decorated("[-1.0, +inf]_def") === DecoratedInterval(Interval(-1.0,Inf), def)
    ...
    @test_skip @decorated("[-Infinity, 1.000 ]_trv") === DecoratedInterval(Interval(-Inf,1.0), trv)
    ...
    @test_skip @decorated("2.500?5de-5") === DecoratedInterval(Interval(0x1.a2976f1cee4d5p-16,0x1.a36e2eb1c432dp-16), com)
    ...
    @test_skip isnai(@decorated("[ Nai  ]_ill"))

    @test_skip isnai(@decorated("[ Nai  ]_trv"))

    @test_skip isnai(@decorated("[ Empty  ]_ill"))

    @test_skip isnai(@decorated("[  ]_com"))

    @test_skip isnai(@decorated("[,]_com"))

    @test_skip isnai(@decorated("[   Entire ]_com"))

    @test_skip isnai(@decorated("[ -inf ,  INF ]_com"))

    @test_skip isnai(@decorated("[  -1.0  , 1.0]_ill"))

    @test_skip isnai(@decorated("[  -1.0  , 1.0]_fooo"))

    @test_skip isnai(@decorated("[  -1.0  , 1.0]_da"))

    @test_skip isnai(@decorated("[-1.0,]_com"))

    @test_skip isnai(@decorated("[-Inf, 1.000 ]_ill"))

    @test_skip isnai(@decorated("[-I  nf, 1.000 ]"))

    @test_skip isnai(@decorated("[-Inf, 1.0  00 ]"))

    @test_skip isnai(@decorated("[-Inf ]"))

    @test_skip isnai(@decorated("[Inf , INF]"))

    @test_skip isnai(@decorated("[ foo ]"))

    @test_skip isnai(@decorated("0.0??_com"))

    @test_skip isnai(@decorated("0.0??u_ill"))

    @test_skip isnai(@decorated("0.0??d_com"))

    @test_skip isnai(@decorated("0.0??_com"))

    @test_skip isnai(@decorated("[1.0,2.0"))

    @test_broken @decorated("[1.0000000000000002,1.0000000000000001]") === DecoratedInterval(Interval(1.0,0x1.0000000000001p+0), com)
    ...
    @test_broken @decorated("[0x1.00000000000002p0,0x1.00000000000001p0]") === DecoratedInterval(Interval(1.0,0x1.0000000000001p+0), com)
...
@testset "minimal_set_dec_test" begin
    ...
    @test_broken isnai(DecoratedInterval(emptyinterval(), ill))

    @test_broken isnai(DecoratedInterval(interval(-Inf,3.0), ill))

    @test_broken isnai(DecoratedInterval(interval(-1.0,3.0), ill))
