...
# Returns false for proper display of complex intervals
signbit(::Interval) = false

"""
    interval_signbit(x::Interval)
...
# Examples
```jldoctest
julia> interval_signbit(@interval(-4))
[1, 1]

julia> interval_signbit(@interval(5))
[0, 0]

julia> interval_signbit(@interval(-4,5))
[0, 1]
```
"""
function interval_signbit(a::Interval)
    isempty(a) && return emptyinterval(a)
...
for Typ in (:Interval, :Real, :Float64, :Float32, :Signed, :Unsigned)
    @eval begin
        copysign(a::$Typ, b::Interval) = abs(a)*(1-2interval_signbit(b))
        flipsign(a::$Typ, b::Interval) = a*(1-2interval_signbit(b))
    end
end
